...
try {
     ...
     */
    if (!empty($search)) {
        $sql .= " WHERE (project_code LIKE ? OR code LIKE ? OR project_name LIKE ?)";
        
        $likeSearch = "%" . $search . "%";
        
        // Push parameters 3 times (for the 4 placeholders above)
        $params[] = $likeSearch;
        $params[] = $likeSearch;
        $params[] = $likeSearch;
     ...
     */
    $sql .= " ORDER BY project_name ASC LIMIT 100";
