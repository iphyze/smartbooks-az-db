...
function writeDetailsBlock($sheet, int $start, string $heading, array $items, string $color): array
{
    ...
    styleRange($sheet, "A{$row}:F{$row}", ['bold' => true, 'color' => 'FFFFFFFF', 'bg' => $color, 'border' => true]);
    $row++;
    $headers = ['Source', 'Date', 'Reference', 'Description', 'Amount', 'Category / Remarks'];
    foreach ($headers as $i => $h) $sheet->setCellValue(Coordinate::stringFromColumnIndex($i + 1) . $row, $h);
    styleRange($sheet, "A{$row}:F{$row}", ['bold' => true, 'color' => 'FFFFFFFF', 'bg' => 'FF009E87', 'border' => true]);
    $row++;
    $firstData = $row;
    if (!$items) {
        $sheet->setCellValue("A{$row}", 'No items');
        $sheet->mergeCells("A{$row}:F{$row}");
        styleRange($sheet, "A{$row}:F{$row}", ['italic' => true, 'color' => 'FF7AADA6', 'border' => true]);
        $row++;
    } else {
        foreach ($items as $i => $l) {
            $sheet->setCellValue("A{$row}", $l['_source']);
            $sheet->setCellValue("B{$row}", fmtD($l['txn_date']));
            $sheet->setCellValue("C{$row}", $l['reference']);
            $sheet->setCellValue("D{$row}", $l['description']);
            $sheet->setCellValue("E{$row}", (float)$l['amount']);
            $sheet->setCellValue("F{$row}", trim(($l['category_name'] ?: ($l['bank_only_type'] ?? '')) . ' ' . ($l['journal_note'] ? '- ' . $l['journal_note'] : '')));
            styleRange($sheet, "A{$row}:F{$row}", ['bg' => $i % 2 === 0 ? 'FFFFFFFF' : 'FFF8FCFB', 'border' => true, 'wrap' => true]);
            $row++;
        }
    }
    $sheet->setCellValue("D{$row}", 'Total');
    $sheet->setCellValue("E{$row}", $row > $firstData ? '=SUM(E' . $firstData . ':E' . ($row - 1) . ')' : 0);
    moneyFmt($sheet, "E{$firstData}:E{$row}");
...
try {
    ...
    foreach (['A' => 12, 'B' => 14, 'C' => 18, 'D' => 62, 'E' => 18, 'F' => 40] as $col => $w) $s2->getColumnDimension($col)->setWidth($w);
    ...
    // Category sheets from classified items
    $byCat = [];
    ...
    ksort($byCat);
    foreach ($byCat as $cat => $items) writeCategorySheet($ss->createSheet()->setTitle(cleanSheetName($cat)), $cat, $items, $recon);
