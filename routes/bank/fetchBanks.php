...
try {
    ...
    requireAnyPermission($conn, $userData, ['bank.view', 'invoice.create', 'invoice.edit', 'invoice.payment_record'], 'You do not have permission to load bank reference data.');
     ...
     */
    if (!empty($search)) {
        $sql .= " WHERE (bank_name LIKE ? OR account_name LIKE ? OR account_currency LIKE ? OR CAST(account_number AS CHAR) LIKE ?)";
     ...
     */
    $sql .= " ORDER BY account_name ASC LIMIT 100";
