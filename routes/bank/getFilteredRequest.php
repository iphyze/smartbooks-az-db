...
try {
    ...
    requirePermission($conn, $userData, 'bank.view', 'You do not have permission to view bank accounts.');
     ...
     * Base query
     */
    $baseQuery = "FROM bank_table WHERE 1=1";
    $params = [];
    $types  = "";

    /**
     * Search filter
     * Searching across account_name, bank_name, account_currency, and account_number
     */
    if ($search) {
        $baseQuery .= " AND (account_name LIKE ? OR bank_name LIKE ? OR account_currency LIKE ? OR CAST(account_number AS CHAR) LIKE ?)";
        $likeSearch = "%" . $search . "%";
