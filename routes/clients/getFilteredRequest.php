...
try {
    ...
    requirePermission($conn, $userData, 'client.view', 'You do not have permission to view clients.');
     ...
     * Base query
     */
    $baseQuery = "FROM clients_table WHERE 1=1";
    $params = [];
    $types  = "";

    /**
     * Search filter
     * Searching across clients_name, clients_email, clients_address, clients_number, and clients_id
     */
    if ($search) {
        $baseQuery .= " AND (clients_name LIKE ? OR clients_email LIKE ? OR clients_address LIKE ? OR clients_number LIKE ? OR CAST(clients_id AS CHAR) LIKE ?)";
        $likeSearch = "%" . $search . "%";
